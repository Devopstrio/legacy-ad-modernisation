...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Transformation Center', path: '/', icon: LayoutDashboard },
    { name: 'AD Inventory', path: '/inventory', icon: Users },
    { name: 'Trust Relationships', path: '/trusts', icon: Share2 },
    { name: 'Security Posture', path: '/security', icon: ShieldAlert },
    { name: 'Modernisation Planner', path: '/migration', icon: Zap },
    { name: 'Hybrid Connectivity', path: '/hybrid', icon: Cloud },
    { name: 'Protocol Analysis', path: '/protocols', icon: History },
    { name: 'GPO Analytics', path: '/gpo', icon: FileText },
    { name: 'Risk Scorecard', path: '/risks', icon: TrendingUp },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-blue-400 bg-clip-text text-transparent">AD.Modern</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-500 truncate">Modernisation Lead</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search domains, GPOs, service accounts, migration tasks..." 
